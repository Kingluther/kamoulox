...
document.getElementById('btn-lancer').addEventListener('click', async () => {
    await set(ref(db, 'gamestate'), {
        ...
        roles: (gameState.roles) || null, buzzerWinner: null,
    });
    setTimeout(async () => {
        const s = await fresh();
...
async function buildPresentationScript(nomJ1, nomJ2) {
    ...
    if (ouv.anim_intro) {
        const t = ouv.anim_intro.split('[J1]').join(nomJ1).split('[J2]').join(nomJ2).split('[joueur actuel]').join(nomActuel);
    ...
    if (ouv.tour_final) {
        ...
        const t = ouv.tour_final.split('[J1]').join(nomJ1).split('[J2]').join(nomJ2).split('[joueur actuel]').join(finalTurn === realOfLabel.J1 ? nomJ1 : nomJ2);
        script.push({ role: 'ANIM', text: t, segment: 'CHIFOUMI' });
    }
...
function renderJeuBar() {
    ...
    // Défi minuté en cours : on masque la barre normale, un seul bouton "Valider la prestation"
    document.getElementById('bar-jeu-normale').classList.toggle('hidden', !!gameState.defiMinute || !!gameState.alerteDouble);
    document.getElementById('bar-defi-minute').classList.toggle('hidden', !gameState.defiMinute);
    document.getElementById('bar-alerte-double').classList.toggle('hidden', !gameState.alerteDouble);
    if (gameState.alerteDouble) {
        document.getElementById('alerte-consigne-anim').innerText = gameState.alerteDouble.consigne_animateur || '';
        return;
    }
    if (gameState.defiMinute) {
        document.getElementById('defi-consigne-anim').innerText = gameState.defiMinute.consigne_animateur || 'Attendez la prestation du joueur…';
        return;
    }
...
// Certaines transitions ne peuvent apparaître que si une demi-phrase ou une phrase
// courte précise vient d'être jouée (cf. cartes mystère conditionnelles / événements
// plateau conditionnels). On les exclut du tirage sinon.
function transitionsDisponibles(s) {
    const dernier = (s.history || [])[(s.history || []).length - 1];
    const dernierTexte = dernier ? dernier.text : '';
    const conditionnelles = new Set();
    (gameData.evenements_plateau_imitation || []).forEach(e => {
        if (e.declencheur_demi_phrase && dernierTexte.toLowerCase().includes(e.declencheur_demi_phrase.toLowerCase())) {
            conditionnelles.add(e.source);
        }
        if (e.declencheur_phrase_courte && dernierTexte.toLowerCase().includes(e.declencheur_phrase_courte.toLowerCase())) {
            conditionnelles.add(e.source);
        }
    });
    // Note : les entrées vraiment conditionnelles (Castaldi, pain à la mouille, Tonton Mayonnaise)
    // ne sont de toute façon plus dans la banque 'transitions' à plat (retirées) — elles sont
    // uniquement piochées via cette liste quand la condition est remplie.
    return Array.from(conditionnelles);
}
...
async function avancerTransition() {
    ...
    const cur = s.turn;
    const specifiques = transitionsDisponibles(s);
    let evt = null, raw;
    if (specifiques.length) {
        evt = (gameData.evenements_plateau_imitation || []).find(e => e.source === specifiques[0]);
    }
    if (evt) {
        await lancerEvenementPlateau(s, evt, cur);
        return;
    }
    raw = pick(gameData.transitions);
    const evtSpecial = (gameData.evenements_speciaux || []).find(e => e.declencheur === raw && e.type === 'alerte_double_imitation_animateur');
    if (evtSpecial) {
        await lancerAlerteDouble(s, evtSpecial, cur);
        return;
    }
    // si la transition piochée correspond à un défi minuté générique (mini_defis_imitation), on bascule dessus
    const defi = (gameData.mini_defis_imitation || []).find(m => m.transition === raw);
    if (defi) {
        await lancerDefiMinute(s, defi, raw, cur);
        return;
    }
...
async function lancerAlerteDouble(s, evt, cur) {
    await update(ref(db, 'gamestate'), Object.assign(
        {
            alerteDouble: {
                consigne_joueurs: evt.consigne_joueurs, consigne_animateur: evt.consigne_animateur,
                duree_secondes: evt.duree_secondes || 3, pourJoueurSuivant: otherOf(cur),
            },
        },
        addLog(s, 'ANIM', evt.declencheur)
    ));
}
document.getElementById('btn-valider-alerte').addEventListener('click', async () => {
    const s = await fresh();
    const a = s.alerteDouble;
    if (!a) return;
    await update(ref(db, 'gamestate'), { alerteDouble: null, turn: a.pourJoueurSuivant, turnCount: (s.turnCount || 0) + 1 });
});

async function lancerDefiMinute(s, defi, rawTransition, cur) {
    const text = fillNames(rawTransition, s, cur, otherOf(cur));
    await update(ref(db, 'gamestate'), Object.assign(
        {
            defiMinute: {
                pourJoueur: cur, consigne_joueur: defi.consigne_joueur, indice: null,
                consigne_animateur: defi.consigne_animateur, duree_secondes: defi.duree_secondes || 3,
                passeAuAdversaire: true,
            },
        },
        addLog(s, 'ANIM', text)
    ));
...
async function lancerEvenementPlateau(s, evt, cur) {
    const boardPatch = evt.case_plateau ? signalBoardNamedCell(evt.case_plateau)
        : (evt.deplacement_aleatoire_cases || evt.deplacement_simple) ? signalBoardRandom() : {};
    const text = evt.source; // flavor déjà présent dans l'historique via la transition normale au besoin
    const patchObj = Object.assign({}, boardPatch);
    if (evt.consigne_joueur) {
        patchObj.defiMinute = {
            pourJoueur: cur, consigne_joueur: evt.consigne_joueur, indice: evt.indice || null,
            consigne_animateur: evt.consigne_animateur, duree_secondes: evt.duree_secondes || 3,
            passeAuAdversaire: !evt.garde_la_main,
        };
    } else {
        patchObj.turn = evt.garde_la_main ? cur : otherOf(cur);
        patchObj.turnCount = (s.turnCount || 0) + 1;
    }
    await update(ref(db, 'gamestate'), Object.assign(patchObj, addLog(s, 'ANIM', fillNames(text, s, cur, otherOf(cur)))));
}
...
document.getElementById('btn-valider-prestation').addEventListener('click', async () => {
    ...
    if (!d) return;
    const next = d.passeAuAdversaire ? otherOf(d.pourJoueur) : d.pourJoueur;
    await update(ref(db, 'gamestate'), { defiMinute: null, turn: next, turnCount: (s.turnCount || 0) + 1 });
});
...
onValue(ref(db, 'gamestate/tentativeRequest'), async (snap) => {
    ...
    if (t.type === 'carte_mystere_libre' || t.type === 'carte_mystere_libre_ref') {
        ...
        await update(ref(db, 'gamestate'), { history: hist, turn: adv });
    } else if (t.type === 'larsen_double_alerte') {
        const evt = (gameData.evenements_speciaux || []).find(e => e.type === 'alerte_double_imitation_animateur');
        if (evt) await lancerAlerteDouble(s, evt, cur);
