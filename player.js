...
function renderSelection() {
    ...
    if (!gameData.personnages || !gameData.personnages.length) {
        el('character-list').innerHTML = '<p>Chargement des personnages…</p>';
        return;
    }
    if (!selectionPool || !selectionPool.length) {
...
function renderGame() {
    ...
    // Kamoulox : une fois les seuils atteints ET que le joueur perdant a cliqué ET sur sa 1ère moitié, il se fige
    if (gameState.kamouloxStuckActive) {
...
function renderStuck() {
    ...
    const b = document.createElement('button');
    b.className = 'card-phrase selected'; b.innerText = gameState.kamouloxFrozenPhrase || '…';
    grid.appendChild(b);
...
function renderKamouloxOpportunity() {
    el('phrase-pool').innerHTML = `<p>${gameState.kamouloxFrozenPhrase || ''} et…</p><p>Votre adversaire semble bloqué…</p>`;
    el('btn-et').classList.add('hidden');
    el('btn-courte').classList.add('hidden');
    el('btn-contre').classList.add('hidden');
    ...
    kb.onclick = async () => {
        const s = await fresh();
        const stuckText = s.kamouloxFrozenPhrase || '…';
...
function renderNormalTurn() {
    if (gameState.contreForce === myId) {
        if (navigator.vibrate) navigator.vibrate([200, 100, 200, 100, 200]);
        el('phrase-pool').innerHTML = '<p>Vous devez contrer !</p>';
        el('btn-et').classList.add('hidden'); el('btn-courte').classList.add('hidden');
        const cb = el('btn-contre');
        cb.classList.remove('hidden'); cb.classList.add('glow'); cb.innerText = 'JE CONTRE !';
        cb.onclick = async () => {
            const phrase = (gameData.je_contre && pick(gameData.je_contre)) || 'Je contre !';
            await submitLine(myId, phrase);
            const s = await fresh();
            await patch({ [`contreUsed/${myId}`]: true, contreForce: null, turnCount: (s.turnCount || 0) + 1 });
        };
        el('btn-kamoulox').classList.add('hidden');
        return;
    }
    if (navigator.vibrate) navigator.vibrate(200);
    el('btn-kamoulox').classList.remove('glow');
    ...
    if (!firstHalf) {
        ...
        el('btn-et').classList.remove('hidden');
        el('btn-et').onclick = async () => {
            const chosen = document.querySelector('#phrase-pool .card-phrase.selected');
            if (!chosen) return;
            firstHalf = chosen.dataset.value;
            if (kamouloxReady(gameState) && gameState.turn === gameState.loser && myId === gameState.loser) {
                await patch({ kamouloxFrozenPhrase: firstHalf, kamouloxStuckActive: true });
                return;
            }
...
async function checkConditionalTrigger(texteJoue) {
    ...
    const evt = (gameData.defis_chronometres || []).find(e =>
    ...
    if (evt) {
        await patch({
            defiMinute: {
                pourJoueur: myId, quiExecute: evt.qui_execute, consigne: evt.consigne, indice: evt.indice || null,
                consigneAnimateur: evt.consigne_animateur_defaut || 'Attendez que le joueur ait fini',
                duree_secondes: evt.duree_secondes || 3, apres: evt.apres || 'transition_normale',
            },
        });
        return true;
    }
