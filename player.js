...
function renderGame() {
    ...
    // Kamoulox : le joueur perdant désigné est bloqué / l'autre peut conclure
    const prereq = !!gameState.carteMystereUsed && !!(gameState.contreUsed && (gameState.contreUsed.J1 || gameState.contreUsed.J2));
    ...
    if (gameState.turn !== myId) { show('wait-screen'); setWait('En attente de l\'adversaire…'); return; }
...
function renderOffers(active) {
    ...
    const offerNow = turnCount % 3 === 2;
    if (!offerNow) { btn.classList.add('hidden'); return; }
    ...
    btn.onclick = async () => {
        const offer = courteOffer; courteOffer = null;
        if (offer.kind === 'tentative') {
            ...
            await patch({
                ...
                contrePending: false,
                courteAwaitingDecision: { by: myId, text: offer.text },
                hapticFor: otherId,
            });
        }
    };
...
function renderContre(active) {
    ...
    btn.onclick = async () => {
        ...
        const s = await fresh();
        await patch({ [`contreUsed/${myId}`]: true, turnCount: (s.turnCount || 0) + 1 });
        firstHalf = null; poolA = []; poolB = [];
    };
}
